...
div
  .ExpiredProductList
    .level.toolbar(:class="{ 'shadow-divider': formPanelOpen }")
      .level-left
        .level-item.page-title.subtitle.is-5 Listing Expired Products ({{ filteredItemsData.length }})
      //- .level-item
      //-     div.search
    ...
    el-table.p-0.expiredproductslist(
      ref="items-table",
...
export default {
  mounted() {
    this.clearExpiredProducts();
    this.loading = true;
    this.preloadItemsList();
    this.handleBottomScroll(
      document.querySelector('.expiredproductslist .el-table__body-wrapper')
    );
  },
  ...
  computed: {
    ...mapState('products', ['expiredProducts']),
  },
  components: {
    Loading,
    InfiniteLoading,
