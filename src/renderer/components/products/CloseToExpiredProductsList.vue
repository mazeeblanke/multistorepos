...
div
  .ExpiredProductList
    .level.toolbar(:class="{ 'shadow-divider': formPanelOpen }")
      .level-left
        .level-item.page-title.subtitle.is-5 Listing Close to Expired Products ({{ filteredItemsData.length }})
      //- .level-item
      //-     div.search
    ...
    Loading(loading-text="Loading close to expired products" v-if="(loading && !filteredItemsData.length) || isSearching", :style="{ height: '400px' }")
    el-table.CloseToExpiredProductList(
      ref="items-table",
...
export default {
  mounted() {
    this.clearCloseToExpiredProducts();
    this.loading = true;
    this.preloadItemsList();
    this.handleBottomScroll(
      document.querySelector('.CloseToExpiredProductList .el-table__body-wrapper')
    );
  },
  ...
  watch: {
    closeToExpiredProducts(newValue) {
      this.items = newValue;
  ...
  methods: {
    ...
    ...mapActions('products', [
      ...
      'clearCloseToExpiredProducts',
      'clearProductEnquiries',
    ]),
    ...mapMutations('products', {
      setItems: 'SET_CLOSE_TO_EXPIRED_PRODUCTS',
    }),
    ...mapMutations('products', [
      'ADD_ENQUIRY',
    ]),
  ...
  computed: {
    ...mapState('products', ['closeToExpiredProducts']),
  },
  components: {
    Loading,
    InfiniteLoading,
